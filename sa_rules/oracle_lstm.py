...
if os.path.isfile('autoencoder_model.keras'):
    ...
    print("Model loaded")
    threshold = np.load('reconstruction_threshold.npy')
...
for index in range(num_features):
    ...
    ax[index].plot(reconstructed_feature.mean(axis=1), label="Reconstructed (avg over timesteps)")
    ax[index].scatter(np.where(anomalies[:, index])[0], original_feature.mean(axis=1)[anomalies[:, index]], c='red', s=10)
...
for index in range(num_features):
    ...
    ax[index].plot(original_feature.mean(axis=1), label="Original (avg over timesteps)")
    ax[index].plot(reconstructed_feature.mean(axis=1), label="Reconstructed (avg over timesteps)")
    ax[index].scatter(np.where(anomalies)[0], original_feature.mean(axis=1)[anomalies], c='red', s=10)
    ax[index].set_xlabel("Sample")
    ax[index].set_ylabel(f"Average Feature Value {combined_mapping_dict[index]}")
    ax[index].legend()
plt.show()